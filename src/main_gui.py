...
class GNSSTestTool:
    ...
    # noinspection PyCompatibility
    def parse_nmea_from_log(self,file_path, console_widget):
        """
    ...
    def process_nmea_log(self, file_path, log_folder, timestamp, reference_point=None, stop_event=None, console_widget=None):
        ...
        try:
            parsed_sentences, nmea_data = self.parse_nmea_from_log(file_path, console_widget)
        except Exception as e:
            logging.error(f"Error during parsing NMEA log file: {file_path}. Exception: {e}")
            if console_widget:
                self.append_to_console_specific(console_widget, f"Error during parsing NMEA log file: {file_path}. Exception: {e}")
            return
