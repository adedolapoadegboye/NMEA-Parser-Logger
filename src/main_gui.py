...
class GNSSTestTool:
    def __init__(self, root):
        ...
        self.table_data = {}  # To store summary table rows for all devices
    ...
    def create_widgets(self):
        ...
        style.map(
            "TNotebook.Tab",
            background=[("selected", "#2A6EBB")],  # Soft blue for active tab
        ...
        self.results_notebook.add(self.accuracy_tab, text="Accuracy")
        ...
        accuracy_summary_frame = ttk.LabelFrame(self.accuracy_tab, text="Summary")
        ...
        self.summary_table = ttk.Treeview(accuracy_summary_frame, columns=columns, show="headings")

        # Configure columns and headings
        for col in columns:
            self.summary_table.heading(col, text=col)
            self.summary_table.column(col, anchor="center", width=120)

        # Pack the Treeview widget
        self.summary_table.pack(fill="x", expand=True)
        ...
        self.results_notebook.add(map_tab, text="Map")

        map_frame = ttk.LabelFrame(map_tab, text="Map View")
        map_frame.pack(fill="both", expand=True, padx=10, pady=10)
        map_placeholder = ttk.Label(map_frame, text="Map with device location and path will be displayed here.")
        map_placeholder.pack(padx=20, pady=20)

        # Altitude Tab
        altitude_tab = ttk.Frame(self.results_notebook)
        self.results_notebook.add(altitude_tab, text="Altitude")

        altitude_frame = ttk.LabelFrame(altitude_tab, text="Altitude Information")
        altitude_frame.pack(fill="both", expand=True, padx=10, pady=10)
        altitude_placeholder = ttk.Label(altitude_frame, text="Altitude data over time will be displayed here.")
        altitude_placeholder.pack(padx=20, pady=20)

        # Speed Over Ground Tab
        speed_tab = ttk.Frame(self.results_notebook)
        self.results_notebook.add(speed_tab, text="Speed Over Ground")

        speed_frame = ttk.LabelFrame(speed_tab, text="Speed Over Ground")
        speed_frame.pack(fill="both", expand=True, padx=10, pady=10)
        speed_placeholder = ttk.Label(speed_frame, text="Speed data over ground will be displayed here.")
        speed_placeholder.pack(padx=20, pady=20)

        # Course Over Ground Tab
        course_tab = ttk.Frame(self.results_notebook)
        self.results_notebook.add(course_tab, text="Course Over Ground")

        course_frame = ttk.LabelFrame(course_tab, text="Course Over Ground")
        course_frame.pack(fill="both", expand=True, padx=10, pady=10)
        course_placeholder = ttk.Label(course_frame, text="Course data over ground will be displayed here.")
        course_placeholder.pack(padx=20, pady=20)

        # Dilution of Precision Tab
        dop_tab = ttk.Frame(self.results_notebook)
        self.results_notebook.add(dop_tab, text="Dilution of Precision")

        dop_frame = ttk.LabelFrame(dop_tab, text="Dilution of Precision")
        dop_frame.pack(fill="both", expand=True, padx=10, pady=10)
        dop_placeholder = ttk.Label(dop_frame, text="Dilution of precision (DOP) data will be displayed here.")
        dop_placeholder.pack(padx=20, pady=20)
    ...
    def read_nmea_data(self, port, baudrate, timeout, duration, log_folder, timestamp, reference_point=None, stop_event=None, console_widget=None):
        ...
        if cep_value:
            ...
            self.update_summary_table(f"Device-{port}", cep_value)
    ...
    def update_summary_table(self, device_name, cep_stats):
        ...
        if not hasattr(self, "summary_table"):
            columns = ["Device", "Fixes", "CEP50 (m)", "CEP68 (m)", "CEP95 (m)", "CEP99 (m)", "Reference Point"]
            self.summary_table = ttk.Treeview(self.accuracy_summary_frame, columns=columns, show="headings")
            for col in columns:
                self.summary_table.heading(col, text=col)
                self.summary_table.column(col, anchor="center", width=120)
            self.summary_table.pack(fill="x", expand=True)
        ...
        if device_name in self.table_data:
            ...
            for item in self.summary_table.get_children():
                if self.summary_table.item(item, "values")[0] == device_name:
                    self.summary_table.delete(item)
                    break

        # Insert the updated or new row
        self.summary_table.insert("", "end", values=row)
        self.table_data[device_name] = row  # Save the row in the dictionary
    ...
    def clear_accuracy_plot(self):
        ...
        Clears the figure and reinitialize the axes.
        """
        # Reset the Matplotlib figure if it exists and is initialized
        if hasattr(self, "fig") and self.fig is not None:
            self.fig.clear()  # Clear the figure
            self.ax = self.fig.add_subplot(111)  # Recreate the subplot with default settings
            ...
            # Redraw the canvas if it exists and is initialized
            if hasattr(self, "canvas") and self.canvas is not None:
                self.canvas.draw()

        # Clear internal data structures related to plotting if initialized
        if hasattr(self, "device_plot_data") and self.device_plot_data is not None:
            self.device_plot_data = {}  # Reset device plot data dictionary
