...
class NMEAData:
    ...
    def write_to_excel(self, port, baudrate, timestamp, cep_value, filename="nmea_data"):
        """
        Write NMEA parsed data, summary statistics (CEP), and individual data points with distances to an Excel file.
        ...
        try:
            ...
            if ref_point:
                formatted_ref_point = f"({float(ref_point[0]):.6f}, {float(ref_point[1]):.6f})"
            else:
                formatted_ref_point = ''
